...
func TestNewCloudInit(t *testing.T) {
	...
	assert.Equal("", cmd.Flag(types.FlagNameKubeEdgeVersion).Value.String())
	assert.Equal("", cmd.Flag(types.FlagNameAdvertiseAddress).Value.String())
	assert.Equal(types.DefaultKubeConfig, cmd.Flag(types.FlagNameKubeConfig).Value.String())

	assert.Equal("[]", cmd.Flag(types.FlagNameSet).Value.String())
	assert.Equal("", cmd.Flag(types.FlagNameProfile).Value.String())
